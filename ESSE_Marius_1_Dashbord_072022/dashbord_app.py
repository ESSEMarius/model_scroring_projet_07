...
def main() :
    ...
    id_client = list(data.index.values)
    clf = load_model()
    ...
    st.title('Tableau de bord interactif de “scoring crédit”')
    ...
    #AMT CREDIT
    st.sidebar.markdown("<u>Montant moyen des prêts (USD) :</u>", unsafe_allow_html=True)
    st.sidebar.text(credits_moy)
    
    #PieChart
    #st.sidebar.markdown("<u>......</u>", unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(5,5))
    plt.pie(targets, explode=[0, 0.1], labels=['Pas default', 'Default'], autopct='%1.1f%%', startangle=90)
    ...
    st.header("**Affichage des informations sur les clients**")

    if st.checkbox("Afficher les informations relatives aux clients ?"):
        infos_client = identite_client(data, chk_id)
        st.write("**Sexe : **", infos_client["CODE_GENDER"].values[0])
        st.write("**Age : **{:.0f} ans".format(int(infos_client["DAYS_BIRTH"]/365)))
        st.write("**Situation matrimoniale : **", infos_client["NAME_FAMILY_STATUS"].values[0])
        ...
        ax.set(title='Âge du client', xlabel='Age(Year)', ylabel='')
        st.pyplot(fig)
        ...
        st.write("**Annuités de crédit : **{:.0f}".format(infos_client["AMT_ANNUITY_x"].values[0]))
        ...
        ax.set(title='Revenu des clients', xlabel='Income (USD)', ylabel='')
        st.pyplot(fig)
    ...
    st.header("**Analyse des fichiers clients**")
    prediction = load_prediction(sample, chk_id, clf)
    st.write("**Probabilité de défaut : **{:.0f} %".format(round(float(prediction)*100, 2)))
    
    st.markdown("<u>Données clients :</u>", unsafe_allow_html=True)
    st.write(identite_client(data, chk_id))
    
    #Feature importance / description
    if st.checkbox("ID du client {:.0f} importance des caractéristiques ?".format(chk_id)):
        shap.initjs()
        X = sample.iloc[:, :-1]
        X = X[X.index == chk_id]
        number = st.slider("Choisissez un certain nombre de caractéristiques…", 0, 20, 5)
        ...
        if st.checkbox("Besoin d'aide pour la description des fonctionnalités ?") :
            list_features = description.index.to_list()
            feature = st.selectbox('Liste de contrôle des caractéristiques…', list_features)
    ...
    if chk_voisins:
        ...
        st.markdown("<i>Target 1 = Client avec défaut de paiement</i>", unsafe_allow_html=True)
    else:
        st.markdown("<i>…</i>", unsafe_allow_html=True)
